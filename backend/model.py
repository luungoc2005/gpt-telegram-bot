...
INITIAL_PROMPT = """{{bot_name}} is a very smart girl
{{bot_name}} and {{first_name}} are dating
This is a recent discussion between {{first_name}} and {{bot_name}}
---
{{first_name}}: Hello
...
def predict(model, tokenizer, device, first_name, history_items, message):
    ...
    for item in history_items:
        ...
        prompt += f"\n"
        if int(item['from']) == 0:
            prompt += human_prefix + item['message']
        else:
            prompt += bot_prefix + item['message']
